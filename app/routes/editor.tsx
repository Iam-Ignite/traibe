...
export function meta({}: Route.MetaArgs) {
  return [
    { title: "Editor - Mini CMS" },
    { name: "description", content: "Edit your articles" },
  ];
}

// Main Component
export default function Editor({ loaderData, actionData }: Route.ComponentProps) {
  const { tree, currentArticle } = loaderData;
        ...
        <aside className="w-80 bg-white border-r border-gray-200 overflow-y-auto flex-shrink-0">
          <div className="p-4">
            ...
            <div className="space-y-2">
              {tree.map(({ category, articles }: any) => (
                <TreeCategory
                  key={category}
                  category={category}
                  articles={articles}
